...
- **Share the caught-Pokémon collection's `localStorage` key verbatim** between the Field Guide and the game — a Pokémon caught or leveled in the game should show up in the Field Guide's My Pokemon tab (and be evolvable there, reusing its existing Evolve UI, §"caught Pokémon collection" above) with zero extra code, and vice versa. Give the game its own *separate* key for its own state (current board position, badges/gyms beaten, whether a starter's been chosen) — the Field Guide's own `badges` counter is a throwaway in-memory variable, not something a real save should depend on.
...
- **Movement is a die roll plus step-by-step graph traversal**, not "teleport N spaces along a line" — the region map is a graph (§1), not a track, so the player picks a direction at each step (via the map's own edge list) up to the rolled total, with the option to stop early. **Unused movement becomes a resource, not a penalty**: the player can trade forfeited spaces for a same-turn advantage elsewhere (this project's Calli game lets forfeited spaces shift a route's wild-vs-trainer roll in either direction) rather than just losing them.
...
- [ ] (Optional) a playable game mode as its own separate page (not a Field Guide tab), sharing the collection's localStorage key but with its own save-state key; die-roll + graph-step movement with a forfeit-for-advantage option; tile resolution driven by the map node's own fields; every matchup resolved through the existing single-roll battle system; XP = defeated Pokémon's stat total (×1.5 for trainer/gym), split across multiple participants with remainder to the latest-sent; leveling as an `xp` counter plus a `while(xp >= level*10)` loop; one unconditional render call at the end of every state-changing function, not scattered before each early return
